...
make 完, redis-3.2.5/src 目录下会出现编译后的 redis 服务程序 redis-server, 还有用于测试的客户端程序 redis-cli
...
注意这种方式启动redis 使用的是默认配置. 也可以通过启动参数告诉 redis 使用指定配置文件使用下面命令启动.
...
启动 redis 服务进程后, 就可以使用测试客户端程序 redis-cli 和 edis 服务交互了. 比如:
