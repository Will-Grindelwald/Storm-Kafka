...
public class SensorTemperTopology {
	...
	public SensorTemperTopology(Properties configProps) {
		...
		log.info("zkStr = " + zkStr + ", inputTopic_ljc_demo = " + inputTopic_ljc_sensor_temper + ", outputTopic_ljc_demo = " + outputTopic_ljc_sensor_temper + ", zkRoot = " + zkRoot + ", spoutId = " + spoutId_ljc_sensor);
		...
		SpoutConfig spoutConfig = new SpoutConfig(brokerHosts, // 第一个参数 hosts 是上面定义的 brokerHosts
				inputTopic_ljc_sensor_temper,                  // 第二个参数 topic 是该 KafkaSpout 订阅的 topic 名称
				zkRoot,                                        // 第三个参数 zkRoot 是存储消费的 offset(存储在 ZK 中了), 当该 topology 故障重启后会将故障期间未消费的 message 继续消费而不会丢失(可配置)
				spoutId_ljc_sensor                             // 第四个参数 id 是当前 spout 的唯一标识
		);
	...
	public void submit(String topologyName) throws AlreadyAliveException, InvalidTopologyException, AuthorizationException {
		...
		if (topologyName == null || topologyName.isEmpty()) { // 本地运行, 可以看到 log 输出, 用于调试
			topologyName = SensorTemperTopology.class.getSimpleName();
