	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				randValue[no] = randValue[no] + direct * rnd.nextInt(1000) * rnd.nextGaussian() / 10000; // 下一个尽量连续的随机数
				while(randValue[no] > max[no]) randValue[no] -= rnd.nextDouble() / 10000;
				while(randValue[no] < min[no]) randValue[no] += rnd.nextDouble() / 10000;
				value = (int)(randValue[no] * 10000) / 10000.0; // 精度为 4 位小数
				
				msg = (new Date().getTime() * 1000 + System.nanoTime() % 1000000 / 1000) + ":" + value;
				this.producer.send(new ProducerRecord<String, String>(topic, key, key + ":" + msg));
