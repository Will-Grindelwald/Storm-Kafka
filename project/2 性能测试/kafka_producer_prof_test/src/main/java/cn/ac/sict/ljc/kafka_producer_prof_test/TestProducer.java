...
public class TestProducer {
	...
	public static void main(String[] args) {
		TestProducer producer = new TestProducer(args[0]);
		producer.send(args[1]); // args[0] 为要生成的随机消息数
	}

	public TestProducer(String topicStr) {
	...
	private void send(String arg) {
		...
		try {
			for (long nEvents = 1; nEvents <= events; nEvents++) {
				...
				this.producer.send(new ProducerRecord<String, String>(topicStr, nEvents + "", msg));
